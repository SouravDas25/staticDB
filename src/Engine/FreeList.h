...
public:
    ...
    bool is_side_by_side(P_ADDR i, P_ADDR p)
    {
        const FreeNode& fi = this->fl.at(i);
        const FreeNode& fp = this->fl.at(p);
        return fi.get_addr() + fi.get_size() == fp.get_addr() || fp.get_addr() - fi.get_size() == fi.get_addr();
    }
    ...
    void shrinkAdjacent()
    {
        ...
        {
            for ( int64_t p = 0 ; p < v.size() ; ++p )
            {
                if(this->is_side_by_side(i, (P_ADDR) p) && v[i].addr != v[p].addr )
                {
