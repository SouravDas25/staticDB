...
namespace StaticDB {
    ...
    void writeDataSignature(const FileLayer &f){
    ...
    public:
        ...
        bool read(const FileLayer &f) {
            ...
            if (!isDataSignatureMatching(f)) {
            	cout << "INCONSISTENCY : bn data signature did not match " << this->addr() << endl;
                return false;
            }
        ...
        bool hasSufficientValidMemory(){
            return this->size() <= this->memory() && this->addr();
        }
