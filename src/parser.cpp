...
*/
    ...
    string remove_space(const string& str)
	{
	    string rs ;
	    int clevel = 0;
		unsigned int i;bool qou1 = false,qou2 = false;
		for(i=0;i<str.size();i++)
		{
	...
	{
	    int len = str.size();
		int size = db_tuple_comma_count(str,len) + 1 ;bool qou1 = false,qou2 = false;
		...
		for(i=0;i<len+1;i++)
		{
			...
			if( clevel == 0 && qou1 == false && qou2 == false)
			{
				if(str[i] == ',' || str[i] == 0 )
				{
					...
					if(!is_array )
                    {
                        if(!key.size())throw sdb_error("Tuple Should Always Contain Keys For Each Data Value. Try Array[...] For Indexed Data.");
                        if( identifierValidity(key,true) == false )
                        throw sdb_error("Invalid Tuple Key : '"+key+"'");
                    }
                    if(is_array)
                    {
                        if(key.size())throw sdb_error("Array Should Not Contain Any Keys Only Data Accessed by Index Are Allowed.");
                    }
