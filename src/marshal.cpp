...
nosqlRT sdb_execute(staticDB * db,vector<string>& cmd)
{
    ...
    else if(p.cmd == nosql_FIND) //! >>> FIND { projection } IN kilo WHERE { selection } ;
    {
        ...
        {
            if( stricmp(cmd[2],"IN") != 0 ) return noSql_Msg(ERROR_MSG,"An IN Keyword Should Be Present In FIND Query Statement.");
            if( cmd.size() == 6 && stricmp(cmd[4],"WHERE") != 0 ) return noSql_Msg(ERROR_MSG,"A WHERE Keyword Should Be Present Before The Query.");
            p.key = cmd[3];
