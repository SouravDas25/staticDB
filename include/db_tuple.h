...
namespace StaticDB {
    ...
    class db_tuple_var {
		...
		public:
            ...
            explicit db_tuple_var(int32_t value , const string& set_key = empty_string ) : i(-1) , _key(nullptr) , _type(db_NULL)
            {
                this->key(set_key);
                this->type(db_INT);
                i = value;
            }
            explicit db_tuple_var(int64_t value , const string& set_key = empty_string ) : i(-1) , _key(nullptr) , _type(db_NULL)
            {
                this->key(set_key);
                this->type(db_INT);
                i = value;
            }
            explicit db_tuple_var(double value, const string& set_key = empty_string  ) : i(-1) , _key(nullptr) , _type(db_NULL)
            {
                this->key(set_key);
                this->type(db_DOUBLE);
                d = value;
            ...
            explicit db_tuple_var(const string& value , const string& set_key = empty_string ) : i(-1) , _key(nullptr) , _type(db_NULL)
            {
                this->key(set_key);
                this->type(db_STRING);
                s = new string(value);
            }
            explicit db_tuple_var( const tuple&  value , const string& set_key = empty_string ) : i(-1) , _key(nullptr) , _type(db_NULL)
            {
                this->key(set_key);
