        ...
        for (i = start; i < kt.size(); i++) {
            ...
            if (lt == nullptr) {
                ...
                if (kt.get_type(i) == db_INT)
                    ...
                    s = string_sprintf("Tuple Key '%s' is Not a Tuple Or Array Error_no - 1.",
                                       kt.get_string(i - 1).c_str());
                throw sdb_error(s);
            }
            ...
            if ((lt->get_type(ind) != db_TUPLE && lt->get_type(ind) != db_ARRAY) && i < kt.size() - 1) {
                ...
                if (kt.get_type(i) == db_INT)
                    ...
                    s = string_sprintf("Tuple Key '%s' is Not a Tuple Or Array Error_no - 2.",
                                       kt.get_string(i).c_str());
                throw sdb_error(s);
            }
            lt = (lt->get_type(ind) == db_TUPLE || lt->get_type(ind) == db_ARRAY) ? &lt->data[ind].get_tup_arr()
                                                                                  : nullptr;//db_tuplePrint(plt,1)
        }
        //plt->print();
        return plt->get(ind);
    }
