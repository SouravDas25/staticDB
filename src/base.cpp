...
namespace StaticDB {
    ...
    typedef struct _stringBase {
        ...
        P_ADDR base_index_pos;
    } stringBase;

    static void base_cache(baseNode *bn, const File &f);

    static baseNode *baseGetNode(staticDB *db, baseIndex index);

    stringBase *baseCreate(const File &fp, P_ADDR fip) {
...
void basePrint(staticDB * db)
{
    ...
    string base_mem_detail(staticDB *db) {
        ...
        det << " memory : " << memory;
        det << " index_size : " << baseCount(sb) * (sizeof(P_ADDR) + sizeof(baseIndex) + baseNode_utility_disk_size());
        memory = 0;
...
/**  DISK IO START EDIT WITH CAUTION **/
    ...
    static bool read_bn(baseNode *bn, const File &f) {
        if (!f.seek(bn->addr)) {
    ...
    static bool write_bn(baseNode *bn, const File &f) {
        if (!f.seek(bn->addr)) {
            cout << "INCONSISTENCY : bn->addr not seek_able " << bn->addr << endl;
            return false;
        }
    ...
    void base_del_insert(stringBase *sb, P_ADDR addr, int64_t memory) {
        delNode dn;
        if (!addr) return;
        dn.addr = addr;
    ...
    static bool delNode_cmp(delNode i, delNode j) {
    ...
    static P_ADDR get_mem_for_write(stringBase *sb, size_t req_size, int &memory) //! VERY SENSITIVE FUNCTION
    {
        shrink_adjacent(sb);
        vector <delNode> &v = *sb->base_del;
        for (uint64_t i = 0; i < v.size(); ++i) {
    ...
    nosqlRT base_Commit(stringBase *sb, const File &f) {
        //int amt_written = 0;
...
void baseCacheAll(stringBase * sb,FILE *fp)
{
    ...
    void baseWrite(stringBase *sb, const File &f, P_ADDR *pos) {
        shrink_adjacent(sb);
        *pos = sb->base_index_pos;
        ...
        for (map<baseIndex, baseNode *>::iterator it = sb->base->begin(); it != sb->base->end(); ++it) {
            ...
            f.write(&bn->addr, sizeof(P_ADDR)); //! address
        }
...
//#define DEBUG
    ...
    bool baseRead(stringBase *sb, const File &f, P_ADDR pos) {
        ...
        for (i = 0; i < baseNode_count; i++) {
            ...
            f.read(&bn->addr, sizeof(P_ADDR));      //! address of index
#ifdef DEBUG
