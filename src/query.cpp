...
namespace StaticDB {
    ...
    template<class T>
    bool are_almost_equal(T x, T y, int ulp = 0.00000001) {
    ...
    static int db_tuple_cmp(const db_tuple_var &data1, const db_tuple_var &data2) {
        bool int_dubl = ((data1.type() == db_INT && data2.type() == db_DOUBLE) ||
                         (data1.type() == db_DOUBLE && data2.type() == db_INT));
    ...
    bool is_sel_match(staticDB *db, const db_tuple_var &srt, const tuple &sel, int match) {
        if (!srt.is_container())
            throw sdb_error("Data Given To FIND Command Is Not A Container Thus Cannot Be Queried.");
